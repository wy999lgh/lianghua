# -*- coding: utf-8 -*-
"""
网格交易系统 - 统一数据库操作层 (PostgreSQL/SQLite)
Unified database operations for the grid trading system

特性:
- 支持 PostgreSQL 和 SQLite 双后端
- 使用连接池（PostgreSQL）
...
class Database:
    ...
    def _init_postgres(self):
        ...
        try:
            ...
            current_hash = hashlib.md5(
                f"{self.config.postgres_host}:{self.config.postgres_port}:{self.config.postgres_database}:{self.config.postgres_user}".encode()
            ).hexdigest()

            if Database._pool is None or Database._pool_config_hash != current_hash:
                if Database._pool is not None:
                    Database._pool.close()
                password = self.config.get_postgres_password()
                conninfo = f"host={self.config.postgres_host} port={self.config.postgres_port} dbname={self.config.postgres_database} user={self.config.postgres_user} password={password}"
            ...
            raise RuntimeError("PostgreSQL 驱动未安装，请安装 psycopg 和 psycopg_pool")
    ...
    def _init_sqlite(self):
        ...
        Database._sqlite_conn = sqlite3.connect(db_path, check_same_thread=False)
    ...
    def _release_connection(self, conn):
        ...
        if self.backend == "postgres":
            try:
                if not conn.closed:
                    if conn.transaction_status != __import__('psycopg').pq.TransactionStatus.IDLE:
                        conn.rollback()
    ...
    def _execute(self, conn, sql, params=None, fetch=False):
        """执行SQL语句（统一接口）"""
        if self.backend == "postgres":
            cur = conn.cursor()
            if params:
    ...
    def save_data(self, df: pd.DataFrame, table_name: str, if_exists: str = 'append'):
        ...
        try:
            cur = self._execute(conn, MARKET_DATA_TABLE_SCHEMA.format(table_name=table_name))
            ...
            if if_exists == 'replace':
                self._execute(conn, f"DELETE FROM {table_name}")
            ...
            if self.backend == "postgres":
                ...
                for row in df.itertuples(index=False):
                    self._execute(conn, sql, tuple(row))
    ...
    def get_data(self, table_name: str, start_date: Optional[str] = None,
        ...
        try:
            cur = self._execute(conn,
                ...
                "AND column_name IN ('date', 'trade_date')" if self.backend != "postgres" else
                "SELECT column_name FROM information_schema.columns "
            ...
            if not row:
                return pd.DataFrame()
            date_col = row[0] if isinstance(row, (tuple, list)) else row['column_name']

            order = "ASC" if order_by.upper() not in ("ASC", "DESC") else order_by.upper()
            query = (
                f"SELECT * FROM {table_name} WHERE 1=1 "
                + (f" AND {date_col} >= ?" if start_date else "")
                + (f" AND {date_col} <= ?" if end_date else "")
        ...
        try:
            for item in items:
                if self.backend == "postgres":
                    ...
                    self._execute(conn,
                        """INSERT OR REPLACE INTO instrument_basic (instrument_type, code, name, market, source, updated_at)
    ...
    def list_instrument_basic(self, instrument_types: Optional[List[str]] = None,
        ...
        try:
            ...
            cur = self._execute(conn,
                ...
                f"FROM instrument_basic {where} ORDER BY code LIMIT ? OFFSET ?",
                params,
                fetch=True
            )
            return [
    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        try:
            cur = self._execute(conn,
                "SELECT stock_name FROM a_stock_list WHERE stock_code = ?",
                (stock_code,),
                fetch=True
            )
            result = cur[0] if cur else None
    ...
    def search_stocks(self, keyword: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            return [{"stock_code": r[0], "stock_name": r[1]} for r in cur]
        except Exception:
            return []
        finally:
            self._release_connection(conn)
    ...
    def get_name_from_table(self, table_name: str) -> Optional[str]:
        ...
        Args:
            table_name: 表名，如 'etf_daily_159633'、'stock_daily_000001'
        ...
        try:
            ...
            if '.' not in stock_code:
                if len(stock_code) == 6 and stock_code.isdigit():
                    cur = self._execute(conn,
                        "SELECT stock_name FROM a_stock_list WHERE stock_code = ? OR stock_code = ?",
                ...
                cur = self._execute(conn,
                    "SELECT stock_name FROM a_stock_list WHERE stock_code LIKE ?",
                    (f"{stock_code}.%",),
                    fetch=True
                )
                result = cur[0] if cur else None
    ...
    def get_names_from_tables(self, table_names: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            if remaining_codes:
                like_conditions = " OR ".join(["stock_code LIKE ?"] * len(remaining_codes))
    ...
    def get_strategy_configs(self) -> List[Dict]:
        ...
        try:
            ...
            columns = [desc[0] for desc in cur.description] if hasattr(cur, 'description') else [
                'id', 'strategy_name', 'symbol', 'base_price', 'upper_step', 'lower_step',
                'upper_count', 'lower_count', 'max_position', 'min_position',
                'initial_cash', 'buy_quantity', 'sell_quantity', 'created_at'
            ]
            result = []
            for row in cur:
    ...
    def get_backtest_results(self, config_id: Optional[int] = None) -> List[Dict]:
        ...
        try:
            if config_id:
                cur = self._execute(conn,
                    "SELECT * FROM backtest_results WHERE config_id = ? ORDER BY created_at DESC",
                    (config_id,),
                    fetch=True
                )
            else:
            ...
            columns = [desc[0] for desc in cur.description] if hasattr(cur, 'description') else [
                'id', 'config_id', 'strategy_name', 'strategy_type', 'symbol', 'start_date', 'end_date',
                'initial_cash', 'final_cash', 'total_return', 'annual_return', 'sharpe_ratio',
                'sortino_ratio', 'calmar_ratio', 'max_drawdown', 'volatility', 'win_rate',
                'benchmark_return', 'trade_count', 'full_report', 'chart_data', 'created_at'
            ]
    ...
    def get_trade_records(self, config_id: Optional[int] = None) -> List[Dict]:
        ...
        try:
            if config_id:
                cur = self._execute(conn,
                    "SELECT * FROM trade_records WHERE config_id = ? ORDER BY datetime DESC",
                    (config_id,),
                    fetch=True
                )
            else:
            ...
            columns = [desc[0] for desc in cur.description] if hasattr(cur, 'description') else [
                'id', 'config_id', 'datetime', 'signal', 'price', 'amount', 'position', 'commission', 'created_at'
            ]
    ...
    def delete_strategy_config(self, config_id: int) -> bool:
        ...
        conn = self._get_connection()
        try:
            self._execute(conn, "DELETE FROM trade_records WHERE config_id = ?", (config_id,))
            self._execute(conn, "DELETE FROM backtest_results WHERE config_id = ?", (config_id,))
            self._execute(conn, "DELETE FROM strategy_configs WHERE id = ?", (config_id,))
            if self.backend == "postgres":
                conn.commit()
            return True
        except Exception:
    ...
    def execute_sql(self, sql: str, params: tuple = None, fetch: bool = False):
        ...
        conn = self._get_connection()
        try:
            cur = self._execute(conn, sql, params, fetch)
            if self.backend == "postgres":
                conn.commit()
            return cur if fetch else None
        except Exception as e:
            if self.backend == "postgres":
                conn.rollback()
            raise e
