# -*- coding: utf-8 -*-
"""
网格交易系统 - 统一数据库操作层 (纯 PostgreSQL 版本，带连接池)
Unified database operations for the grid trading system

特性:
- 使用 psycopg2 ThreadedConnectionPool（min=5, max=50）
...
class Database:
    ...
    连接池：使用 psycopg2 ThreadedConnectionPool，避免每次请求新建TCP连接
    """

    _pool: Optional[ThreadedConnectionPool] = None
    _pool_config_hash: str = ""
    ...
    def __init__(
        ...
        if Database._pool is None or Database._pool_config_hash != current_hash:
            ...
            Database._pool = ThreadedConnectionPool(
                minconn=5,
                maxconn=50,
                **config.get_postgres_connect_kwargs(),
            )
    ...
    def _init_trading_db(self):
        ...
        try:
            ...
            ALTER TABLE IF EXISTS strategy_configs ADD COLUMN IF NOT EXISTS strategy_name TEXT DEFAULT '';
            ...
            CREATE INDEX IF NOT EXISTS idx_trade_records_datetime ON trade_records (datetime);
            CREATE INDEX IF NOT EXISTS idx_trade_records_config_id ON trade_records (config_id);
            CREATE INDEX IF NOT EXISTS idx_backtest_results_config_id ON backtest_results (config_id);
            """
            for stmt in [s.strip() for s in ddl.split(";") if s.strip()]:
                cur.execute(stmt)
            conn.commit()
        finally:
            self._pg_release(conn)

    # ==================== 连接池管理 ====================

    def _pg_connect(self):
        """从连接池获取数据库连接（替代每次新建连接）"""
        try:
            ...
            raise RuntimeError(f"获取数据库连接失败: {str(e)}") from e

    def _pg_release(self, conn):
        """归还连接到连接池"""
        if conn is None:
            return
        try:
    ...
    def get_name_from_table(self, table_name: str) -> Optional[str]:
        ...
        try:
            # 从表名中提取代码
            # 格式: {type}_daily_{code}，代码可能包含 .SZ/.SH
